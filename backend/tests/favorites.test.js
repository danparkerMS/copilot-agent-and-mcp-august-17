...
describe('Favorites API', () => {
  ...
  it('DELETE /api/favorites/:bookId should remove a book from favorites', async () => {
    ...
    if (!bookId) return; // skip if no favorites
    const res = await request(app)
    ...
    expect(updatedSandra.favorites).not.toContain(bookId);
  });

  it('DELETE /api/favorites/:bookId should be idempotent when book is not a favorite', async () => {
    const token = getToken('sandra');
    const res = await request(app)
